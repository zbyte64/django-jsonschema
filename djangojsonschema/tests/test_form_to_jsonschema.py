...
class TestForm(forms.Form):
    ...
    select_option = forms.ChoiceField(choices=[])
    ...
    a_multiple_choice = forms.MultipleChoiceField()
    a_typed_multiple_choice = forms.TypedMultipleChoiceField()
    a_null_boolean = forms.NullBooleanField()
    a_regex = forms.RegexField(regex=r'<([A-Z][A-Z0-9]*)\b[^>]*>(.*?)</\1>') #matches tags
    a_slug = forms.SlugField()
    a_time = forms.TimeField()

class FormToJsonSchemaTestChase(unittest.TestCase):
    def setUp(self):
